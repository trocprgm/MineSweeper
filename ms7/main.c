...
int wcol, wrow, ghx, gmx, ghy, gmy, px = 10, py = 10, flags = 0, runcond = 1, boxxys = 0, boxxyso = 0, nc = 0, ihi = (gamewidth - 1), jhi = (gameheight - 1);
...
void mossy(int j, int i) {
      ...
      for (int tj = 0; tj < gameheight; tj++) {
        for (int ti = 0; ti < gamewidth; ti++) {
            if (ringfield[tj][ti] == 'V') {
                ...
                for (int a = -1; a <= 1; a++) {
                  for (int b = -1; b <= 1; b++) {
                      if (!(hiddenfield[tj+a][ti+b] == ' ') && safeplace((tj+a), (ti+b))) {
                          showfield((tj+a),(ti+b));
                          // openfield[tj+a][ti+b] = hiddenfield[tj+a][ti+b];
                      }
                  }
                }
    ...
    outfield(ringfield, "ringfield.csv");
}
